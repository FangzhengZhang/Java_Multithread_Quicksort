...
public class Runner {
	...
	static final int DEFULT_THREAD_NUMBER = 4;
...
//		if(args.length == 1) {
//			switch (args[0]) {
//			case "Quicksort":run_Quicksort_starter(null);
//			break;
//			}
//		}
//		
		if(args.length == 2) {
			switch (args[0]) {
			case "Basic": run_Basic_thread_calss(args[1]);
			break;
			//handle the file name only 
			case "Quicksort": run_Quicksort_starter(args[1]);
			break;
			}
			...
			switch (args[0]) {
			case "Quicksort" : run_Quicksort_starter(args);
			break;
			}
		}
	...
	private static int check_is_number(String inp) {
		try {
			...
			System.out.println("Can you check your input agruments?"
					+ " You typed in a non-number in the number position.");
			System.exit(1);
		}
		return 0;
	}
